## PR curve for SAM3 finetuned ep18 (score 0.005, mask 0.5) — COCO-standard matching
...
bb_data <- data.table::fread(
  "coco_instances/sam_ep18_mask_05_score_005/eval_coco_matching/combined_results_with_bb.csv"
) %>%
  as_tibble() %>%
  mutate(model = "SAM3") %>%
  mutate(
...
cat("\n========== AP RESULTS — SAM3 ep18 (score 0.005, mask 0.5) COCO-matching ==========\n")
...
ggsave(
  "figures/pr_curve_sam3_coco_matching.png",
  PR_plt,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
cat("Saved: figures/pr_curve_sam3_coco_matching.png\n")
