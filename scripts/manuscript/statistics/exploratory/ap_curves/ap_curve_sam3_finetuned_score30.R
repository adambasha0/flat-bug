## PR curve for SAM3 fine-tuned model (checkpoint 18, score threshold 0.30, mask threshold 0.50)
## Produces:
##   figures/sam3_ft_checkpoint18_score30_mask_vs_bbox.png  – combined mask vs bbox PR curve
##  R_LIBS_USER=~/R/libs Rscript scripts/manuscript/statistics/ap_curve_sam3_finetuned_score30.R
...
# Load score 0.30 CSV
# --------------------------------------------------------------------------- #
ft30 <- data.table::fread(
  "data/finetuned/lora/anti_hallucination_lora_v3_checkpoint_6_mask_5_score_35_using_greed_eval_with_bb.csv"
) %>%
  as_tibble() %>%
  mutate(model = "SAM3-FT") %>%
  rowwise() %>%
  mutate(
    IoU_bb = {
...
# --------------------------------------------------------------------------- #
# Compute AP50 / AP50-95
# --------------------------------------------------------------------------- #
ap50_mask   <- compute_ap_at_threshold(ft30, 0.5, "IoU")
ap50_bb     <- compute_ap_at_threshold(ft30, 0.5, "IoU_bb")
ap5095_mask <- mean(sapply(seq(0.5, 0.95, 0.05),
                           function(t) compute_ap_at_threshold(ft30, t, "IoU")))
ap5095_bb   <- mean(sapply(seq(0.5, 0.95, 0.05),
                           function(t) compute_ap_at_threshold(ft30, t, "IoU_bb")))

cat("\n========== SAM3 FT mask 5 checkpoint 6 LORA, score threshold 0.35 ==========\n")
cat("IoU (mask):\n")
...
PR_mask <- ft30 %>%
  ...
  arrange(desc(conf)) %>%
  mutate(
    n_tp = sum(result == "TP"), n_fn = sum(result == "FN"),
    tp_cumsum = cumsum(result == "TP"), fp_cumsum = cumsum(result == "FP"),
    recall_curve    = tp_cumsum / (n_tp + n_fn),
    precision_curve = tp_cumsum / (tp_cumsum + fp_cumsum),
    iou_type = "IoU (mask)"
  ) %>% ungroup()
...
PR_combined <- bind_rows(
  PR_mask %>% select(recall_curve, precision_curve, iou_type),
  PR_bb   %>% select(recall_curve, precision_curve, iou_type)
)
...
PR_plt <- PR_combined %>%
  ...
  scale_y_continuous(
    labels = scales::label_percent(.1, drop0trailing = TRUE),
    expand = expansion(), limits = c(0.60, 1),
...
ggsave(
  "figures/sam3_ft_mask_5_checkpoint6_lora_score_35_mask_vs_bbox.png",
  PR_plt,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
cat("Saved: figures/sam3_ft_mask_5_checkpoint6_lora_score_35_mask_vs_bbox.png\n")
