## PR curve for SAM3 fine-tuned model (checkpoint 18, score threshold 0.01, mask threshold 0.50)
## Produces:
##   figures/sam3_ft_checkpoint18_mask_vs_bbox.png  – combined mask vs bbox PR curve
...
# --------------------------------------------------------------------------- #
# Helpers (same as ap_curve_corrected_sam3_005.R)
# --------------------------------------------------------------------------- #
...
ft_data <- data.table::fread(
  "data/finetuned/combined_results_sam3_ft_mask_05_checkpoint18_score_001_greedy_with_bb.csv"
) %>%
  as_tibble() %>%
...
# --------------------------------------------------------------------------- #
# Classify at IoU=0.50 for mask and bbox PR curves
# --------------------------------------------------------------------------- #
...
cat("\n========== SAM3 FINE-TUNED (checkpoint 18, score 0.01) ==========\n")
cat("IoU (mask):\n")
...
PR_plt <- PR_curve_combined %>%
  left_join(
    AP_results %>%
      mutate(
        label = format_ap_label(iou_type, AP50, AP50_95) %>% factor(., .)
      ) %>%
  ...
  scale_y_continuous(
    labels = scales::label_percent(.1, drop0trailing = TRUE),
    expand = expansion(), limits = c(0.80, 1),
...
ggsave(
  "figures/sam3_ft_checkpoint18_mask_vs_bbox.png",
  PR_plt,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
cat("Saved: figures/sam3_ft_checkpoint18_mask_vs_bbox.png\n")
