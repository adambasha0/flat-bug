## PR curve for Flatbug model (score 0.005) — saves PNG for thesis
## Identical computation to ap_curve_corrected_fb.R; saves combined PNG with full y-axis 0–100%.
...
# ── helper functions (same as ap_curve_corrected_fb.R) ────────────────────────
...
# ── Load data ────────────────────────────────────────────────────────────────
bb_data <- data.table::fread("data/fb_score_005_using_fb_eval_with_bb.csv") %>%
  as_tibble() %>%
  ...
  mutate(
    IoU_bb = {
      if (idx_1 != -1 & idx_2 != -1) {
        compute_bbox_iou(parse_bbox(bbox_1), parse_bbox(bbox_2))
      } else {
        NA_real_
      }
    }
...
cat("\n========== AP RESULTS — Flatbug (score 0.001) ==========\n")
...
# ── Combined PNG plot (full 0–100% axes for thesis comparison with SAM3) ─────
...
ggsave(
  "figures/fb_score005_fb_eval_mask_vs_bbox.png",
  PR_plt,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
cat("Saved: figures/fb_score005_fb_eval_mask_vs_bbox.png\n")
