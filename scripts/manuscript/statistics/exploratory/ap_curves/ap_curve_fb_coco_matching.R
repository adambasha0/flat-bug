## PR curve for Flatbug (score 0.005) — COCO-standard matching
## Reads combined_results_with_bb.csv produced by fb_eval_greedy.py --coco-matching
...
# ── Load data ─────────────────────────────────────────────────────────────────
bb_data <- data.table::fread(
  "coco_instances/flatbug_score_005/eval_coco_matching/combined_results_with_bb.csv"
) %>%
  as_tibble() %>%
...
AP_results <- bind_rows(
  AP_curve_mask %>% summarize(model = "Flatbug", iou_type = "IoU (mask)",
                               AP50 = AP[IoU == 0.5], AP50_95 = mean(AP[between(IoU, 0.5, 0.95)])),
  AP_curve_bb   %>% summarize(model = "Flatbug", iou_type = "IoU (bounding box)",
                               AP50 = AP[IoU == 0.5], AP50_95 = mean(AP[between(IoU, 0.5, 0.95)]))
)

cat("\n========== AP RESULTS — Flatbug (score 0.005) COCO-matching ==========\n")
cat(sprintf("  Mask   AP50:     %6.2f %%\n",  AP_results$AP50[AP_results$iou_type == "IoU (mask)"]    * 100))
cat(sprintf("  Mask   AP50-95:  %6.2f %%\n",  AP_results$AP50_95[AP_results$iou_type == "IoU (mask)"] * 100))
cat(sprintf("  BBox   AP50:     %6.2f %%\n",  AP_results$AP50[AP_results$iou_type == "IoU (bounding box)"]    * 100))
cat(sprintf("  BBox   AP50-95:  %6.2f %%\n",  AP_results$AP50_95[AP_results$iou_type == "IoU (bounding box)"] * 100))
cat("=======================================================================\n\n")

# ── Combined PNG (y-axis 0.6–1.0: Flatbug performs well) ─────────────────────
...
PR_plt <- PR_combined %>%
  ...
  scale_y_continuous(
    labels = scales::label_percent(.1, drop0trailing = TRUE),
    expand = expansion(), limits = c(0.60, 1), n.breaks = 9
  ) +
...
ggsave(
  "figures/pr_curve_fb_coco_matching.png",
  PR_plt,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
cat("Saved: figures/pr_curve_fb_coco_matching.png\n")
