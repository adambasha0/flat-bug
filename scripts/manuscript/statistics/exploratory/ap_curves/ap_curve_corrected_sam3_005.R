...
bb_data <- data.table::fread("data/finetuned/combined_results_sam3_fine_tuning_mask_05_score_001_train_using_greedy_eval_with_bb.csv") %>%
  as_tibble() %>%
  mutate(model = "SAM3") %>%
  rowwise() %>%
  mutate(
...
PR_plt <- PR_curve_mask %>% 
  ...
  theme(
    ...
    legend.position.inside = c(0.999, 0.92),
...
ggsave(
  "figures/sam3_base_model_score_001_lora_mask.png", 
  PR_plt,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
ggsave(
  "figures/sam3_base_model_score_001_lora_bb.png", 
  PR_plt_bb,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
...
ggsave(
  "figures/sam3_base_model_score_001_lora_mask_vs_bbox.png",
  PR_plt_combined,
  device = "png",
  width = 4, height = 4,
  scale = 2.75,
