...
# Add the repository root to the path for SAM3 imports. Walk up from this file
# until the directory holding the `sam3` package is found, so the script works
# wherever it is placed in the tree (and falls back to two levels up).
def _find_sam3_root(start):
    d = os.path.abspath(start)
    while True:
        if os.path.isfile(os.path.join(d, 'sam3', 'model_builder.py')):
            return d
...
sys.path.append(_find_sam3_root(os.path.dirname(os.path.abspath(__file__))))
